...
export class AgentBasicController extends BaseAgent {
  ...
  static async createSchema(req: Request, res: Response) {
    ...
    try {
      const result = await apiFetch(createSchema, 'POST', {
        attributes,
        schema_name: name,
  ...
  static async createCredentialDefinition(req: Request, res: Response) {
    ...
    try {
      const result = await apiFetch(createCredentialDefinition, 'POST', {
        schema_id: schemaId,
        tag,
      });
      ...
      if (result) {
        ...
        res
          .status(500)
          .send({ error: 'createing credential definition failed' });
      }
    } catch (error) {
      res.status(500).send({ error: error.message });
    }
  ...
  static async getCredentialDefinition(req: Request, res: Response) {
    ...
    try {
      ...
      if (result) {
        ...
        res
          .status(500)
          .send({ error: 'fetching credential definition failed' });
      }
    } catch (error) {
      res.status(500).send({ error: error.message });
    }
  ...
  static async sendMessage(req: Request, res: Response) {
    ...
    try {
      const result = await apiFetch(sendMessage(connectionId), 'POST', {
        content: message,
      });
      if (result) {
        res.status(200).send(result);
